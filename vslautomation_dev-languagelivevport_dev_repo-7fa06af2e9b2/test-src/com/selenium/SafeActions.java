...
public class SafeActions extends Sync
{
	...
	/**
	 * Method - Safe Method for User get available options from Drop down, waits until the element is loaded and then selects an option from drop down
	 * @param locator
	 * @param waitTime
	 * @return - List<WebElements> (returns List of WebElements -- Size equal to options count)       
	 * @throws Exception
	 */

	public List<WebElement> safeGetOptionsInDropDown(By locator, int... optionWaitTime)
	{ 
		...
		try
		{
			...
			if(isElementPresent(locator, waitTime))
			{
				...
				Select select = new Select(selectElement);
				options=select.getOptions();
				log.info("Get the available options from the dropdown : "+locator);
			}
			else
			{
				log.error("Dropdown element " + locator + " was not found in DOM in time - "+waitTime);
				Assert.fail("Dropdown element " + locator + " was not found in DOM in time - "+waitTime);
			}
		}
		catch(StaleElementReferenceException e)
		{
			log.error("Dropdown element with " + locator + " is not attached to the page document - StaleElementReferenceException");
			Assert.fail("Dropdown element with " + locator + " is not attached to the page document - StaleElementReferenceException");
		}
		catch (NoSuchElementException e)
		{
			log.error("Dropdown element " + locator + " was not found in DOM in time - "+waitTime+" - NoSuchElementException"); 
			Assert.fail("Dropdown element " + locator + " was not found in DOM in time - "+waitTime+" - NoSuchElementException");
		}
		catch(Exception e)
		{
			log.error("Unable to select option from " + locator + " dropdown"+ e);     
			Assert.fail("Dropdown element " + locator + " was not found in DOM in time - "+waitTime);
		}
		return options;
	}
	...
	/**
	 * Purpose- Method For performing drag and drop operations using robots 
	 ...
	 */
	public void dragAndDropwithRobots(By Sourcelocator, By Destinationlocator, int waitTime, boolean isIE) 
	{
		//System.setProperty("apple.awt.fullscreenhidecursor","true");
		double x1 = 0;
		double y1 = 0;

		x1 = Toolkit.getDefaultToolkit().getScreenSize().getWidth()/2;
		y1 =0;// Toolkit.getDefaultToolkit().getScreenSize().getHeight();
		try
		{            
			Robot robotclass = new Robot();
			robotclass.mouseMove((int)Math.round(x1),(int)Math.round(y1));
		}
		catch(AWTException e)
		{
			log.error("Unable to perform Robot actions at dragand drop");
			e.printStackTrace();
		}
		if(isIE)
		{
			Point coordinates=driver.findElement(Destinationlocator).getLocation();
			int x = coordinates.getX();             
			int y = coordinates.getY(); 
			Dimension dimentions = driver.findElement(Destinationlocator).getSize();
	 ...
	 * @function_call - eg: DragAndDrop(By.id(Sourcelocator), By.xpath(Destinationlocator), "MEDIUMWAIT");
	 */
	public void dragAndDropwithRobots2(By Sourcelocator, By Destinationlocator, int waitTime, boolean isIE) 
	{
		//System.setProperty("apple.awt.fullscreenhidecursor","true");
		double x1 = 0;
		double y1 = 0;
	...
	/**
	 * Move the mouse to null area
	 */
	public void moveMouseUsingRobotToOutOfWebPage()
	{
		double x1 = 0;
		double y1 = 0;
	 ...
	 * @function_call - eg: DragAndDrop(By.id(Sourcelocator), By.xpath(Destinationlocator), "MEDIUMWAIT");
	 */
	public void dragAndDropwithRobots1(By Sourcelocator, By Destinationlocator, int waitTime, boolean isIE) 
	{
		//System.setProperty("apple.awt.fullscreenhidecursor","true");
		double x1 = 0;
		double y1 = 0;
	...
	public void safeClickUsingSikuliWithMouseDown(String sImagePath, String sLocatorName,int waitTime)
	{
		File fw;
		try
		{
			...
			if(!sImagePath.contains(":")){
				sImagePath = System.getProperty("user.dir")+sImagePath;
				//				Thread.sleep(2000);
			}
			...
			if(!fw.exists()){
				log.error("Sikuli image : "+sLocatorName+" does not exist in path : "+sImagePath);
				Assert.fail("Sikuli image : "+sLocatorName+" does not exist in path : "+sImagePath);
			}
			...
			//			int a = r.getCenter().getY();
			//				mouse.mouseDown(mouse.getLocation().getX());
			//				mouse.mouseUp();
			//				Thread.sleep(2000);
			//			}
			//			catch(Exception e)
	...
	public void safeTypeUsingSikuli(String sText, int waitTime)
	{
		try
		{
			...
			log.info("Text " + sText +"' is entered using sikuli");
		}
		catch(org.sikuli.api.SikuliRuntimeException e)
		{
			log.error("Text "+ sText + " is not entered" + UtilityMethods.getStackTrace());
			Assert.fail("Text "+ sText + " is not entered");
		}
		catch(Exception e)
		{
			log.error("Text "+ sText + " is not entered"+UtilityMethods.getStackTrace());
