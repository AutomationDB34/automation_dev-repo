...
public class BaseSetup implements TimeOuts {
	...
	//public static String districtLogin = null;
	...
	public void initializeBaseSetup(@Optional("chrome") String browserType,
			...
			@Optional("RC") String executionMode,
			ITestContext context) {
		try {

			if(executionMode.equalsIgnoreCase("RC"))
			{
				log.info("<h3>--------------------Release Candidate execution mode selected. Complete Functional Flows will be covered in this run.--------------------<h3>");
				app.writeProperty("App.ExecutionMode", executionMode.toUpperCase());
			}
			else
			{
				log.info("<h3>--------------------Quick Check execution mode selected. Smoke level Scenarios will be covered in this run.--------------------<h3>");
				app.writeProperty("App.ExecutionMode", executionMode.toUpperCase());
			}
	 ...
	 *          returns false
	 */
	public boolean isIexploreBrowser() {
		return getBrowserName().trim().equalsIgnoreCase("internet explorer");
	}
